...
function getData(city){
    ...
    return function(){
        ...
        fetch(url,{
            ...
            var $info = $doc.find(".fj_table_wrap");


            var $li = $info.find(".fj_table_tr1");
            var quxian_length = $li.length;

            quxian_data.push([
               city.name,
               $li.eq(0).prev().find('td').eq(3).text()
            ]);
            ...
            for(let i = 0 ; i < quxian_length ; i++){
                ...
                var quxian_name = $item.find('td').eq(0).find('a').text();
                var quxian_price = $item.find('td').eq(3).text();
